---
title: "Modelización matemática en control de plagas"
csl: nature.csl
output:
  html_document:
    toc: yes
...
<br/>
<p align="center">
<img height=210 width=550 src="estadisticaBasica_files/biorremediacion.png"> </a>
</p>
<font size="3">
<br/>
...
Los modelos demográficos son herramientas matemáticas que permiten describir y predecir la dinámica de poblaciones biológicas a lo largo del tiempo. Su importancia radica en que simplifican procesos ecológicos complejos en ecuaciones manejables, ayudando a comprender el crecimiento, la regulación y las interacciones entre especies. Entre los modelos más influyentes se encuentran el modelo de Malthus, el modelo logístico o de Verhulst y el modelo presa–depredador de Lotka–Volterra, cada uno capturando distintos mecanismos ecológicos.
<br/><br/>

### **Modelo de Malthus o exponencial**
El modelo de Malthus describe una población que crece sin restricciones, suponiendo:

* recursos ilimitados,
* condiciones ambientales constantes,
* ausencia de interacciones limitantes.

Se formula con la siguiente ecuación diferencial:
\begin{equation}
N' = rN
\end{equation}

donde $r$ es la tasa intrínseca de crecimiento y $N$ el tamaño poblacional.
...
Este modelo predice un crecimiento exponencial indefinido. Es útil para describir fases iniciales de colonización, crecimiento microbiano en laboratorio o poblaciones invadiendo nuevos ambientes. Sin embargo, no considera límites ecológicos, por lo que no es realista a largo plazo.

<br/><br/>

### **Modelo de Verhulst o logístico**
El modelo logístico introduce el concepto de capacidad de carga **(K)**, representando el nivel máximo de población que el ambiente puede sostener debido a recursos limitados o competencia intraespecífica. Su ecuación diferencial es:

\begin{equation}
N' = rN(1- \frac{N}{K})
\end{equation}

Cuando $N$ es pequeño, la población crece casi exponencialmente; cuando se acerca a $K$, la tasa de crecimiento disminuye hasta estabilizarse. La solución particular adopta una forma sigmoide o curva en “S”:

\begin{equation}
N(t) = \frac{K}{1 + \left(\frac{K - N_0}{N_0}\right)e^{-rt}}
\end{equation}

Este modelo es más realista para poblaciones naturales sometidas a límites ecológicos y procesos de autorregulación.

<br/><br/>

### **Modelo de Lotka–Volterra (presa–depredador)**

El modelo de Lotka–Volterra extiende el análisis a dos especies que interactúan: una presa y un depredador. Supone que:

* la presa crece sin depredador,
* el depredador depende exclusivamente de la presa,
* el encuentro entre ambas determina la tasa de consumo.

Las ecuaciones clásicas son:
...
* c: tasa de mortalidad de los depredadores.
* d: tasa de conversión energética.

El sistema produce ciclos poblacionales: aumentos de presas seguidos por aumentos de depredadores y viceversa. Es un modelo fundamental en ecología para entender dinámicas de depredación, control biológico y estabilidad ecológica.

<br/><br/>

# **Simulación demográfica con R**
### **Simulación Modelo de Malthus**
```{r malthus, echo=T, fig.height = 5.8, fig.width = 6, fig.align = "center"}
# Parámetros
N0 <- 10
r  <- 0.3
# Tiempos
...
### **Simulación Modelo de Verhults**
```{r verhulst, echo=T, fig.height = 5.8, fig.width = 6, fig.align = "center"}
# Parámetros
N0 <- 10
r  <- 0.1
K  <- 100
...
### **Simulación Modelo de Lotka–Volterra**
```{r volterra, echo=T, fig.height = 5.8, fig.width = 6, fig.align = "center"}
library(deSolve)
...
Un invernadero está sufriendo un brote/plaga de _Tuta absoluta_. Se ha monitorizado el número de individuos durante varios días antes de aplicar ningún control, con los siguientes resultados:
...
1) Introduce los datos en R y grafica los datos de la plaga a lo largo del tiempo.
2) ¿Parece crecimiento exponencial o logístico? ¿Por qué?

Se sabe que esta especie tiene una tasa de crecimiento $r$ entre 0.1 y 0.9.

3) Simula un modelo (Malthus o Logístico) con diferentes valores para $r$ y razona qué tasa de crecimiento es más probable dados tus datos (ten en cuenta que quizás necesites valores para otros parámetros como $N_0$ o $K$).
4) Según tú modelo, ¿cuántos individuos habrá el día 15?

Una estrategia interesante de control biológico podría ser la instalación de cajas refugio para murciélagos insectívoros, con el objetivo de aumentar la depredación sobre los adultos de la plaga. Aunque el efecto puede ser difícil de cuantificar y no específico de una sola especie, constituye una medida complementaria dentro del manejo integrado de plagas.

5) Simula un sistema de depredador-presa en el que evalúes cuál es el número de depredadores iniciales que deberías introducir para controlar la plaga (es decir, que su abundancia no supere nunca, por ejemplo, los 20 individuos a lo largo del tiempo). Utiliza el siguiente código.
```{r volterra2, echo=T, fig.height = 5.8, fig.width = 6, fig.align = "center"}
# Instalamos el paquete deSolve
...
# Parámetros
parms <- c(a = 0.5, b = 0.01, c = 0.5, d = 0.01)

# Condiciones iniciales: Cambiar número de depredadores e
# interpretar el resultado
iniciales <- c(P = 15, D = 0)

# Resolver
out_lv <- ode(y = iniciales, times = tiempo, func = lotka, parms = parms)
out <- as.data.frame(out_lv)
